...
function M.is_jest_project()
	...
	if
		vim.fn.filereadable(project_root .. "/jest.config.js") == 1
		or vim.fn.filereadable(project_root .. "/jest.config.ts") == 1
	then
		return true
	end
...
function M.is_vitest_project()
	local project_root = M.get_frontend_root()
	if
		vim.fn.filereadable(project_root .. "/vite.config.js") == 1
		or vim.fn.filereadable(project_root .. "/vite.config.ts") == 1
	then
		return true
	end
	return false
