	...
	callback = function()
		if vim.bo.filetype == "sh" or vim.bo.filetype == "bash" then
			for _, formatter in pairs(formatters) do
				setKey("n", formatter.shortcut, function()
					run_script(formatter)
				end, { buffer = true, desc = formatter.desc })

				if formatter.arg_shortcut then
					setKey("n", formatter.arg_shortcut, function()
						run_script(formatter, true)
					end, { buffer = true, desc = formatter.desc .. " with Args" })
				end
			end
		end
	end,
